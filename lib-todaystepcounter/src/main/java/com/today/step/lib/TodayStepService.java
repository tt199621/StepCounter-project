...
public class TodayStepService extends Service implements Handler.Callback {
    ...
    private NotificationManager nm; //通知管理者
    private NotificationApiCompat mNotificationApiCompat;//通知栏对象
...
//        Log.d("--cid",""+ getDeviceID.getDeviceID());
        ...
        OkGo.<String>post("https:www.yuebu.shop/qubu/step/recordNumber")
                ...
                .params("steps",step)//步数
                .params("mileage",km)//km数
                .params("calorie",power)//消耗能量
                .execute(new com.lzy.okgo.callback.StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {
                        Log.d("--service_step",""+response.body());
...
//        JLogger.i(TAG, "cleanDb");
    ...
    private synchronized void updateNotification(int stepCount) {
        if (null != mNotificationApiCompat) {
            ...
            /***************************/
            OkGoUpDataStep(CURRENT_STEP,km,calorie);

            SharedPreferences sp = getSharedPreferences("data", MODE_PRIVATE);
            SharedPreferences.Editor editor = sp.edit();
            editor.putString("tstep",""+CURRENT_STEP);
            editor.putString("km", km);//
            editor.putString("calorie", calorie);
            editor.commit();
