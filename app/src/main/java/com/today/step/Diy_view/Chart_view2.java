...
public class Chart_view2 extends View  {
    ...
    public Chart_view2(Context context) {
        super(context);
        init();
        init2();
        init3();
        ...
        super.onDraw(canvas);
            ...
            if (code == 1) {
                for (int i = 0; i < step.size(); i++) {
                    ...
                    item[i]=LEFT;
                    canvas.drawText(step_data1.get(i) , LEFT - 53, 80, mpaint4);
                    LEFT += space;
                }
                py2=item;
            } else {
            ...
            case MotionEvent.ACTION_UP:
                ...
                for (int i=0;i<item.length;i++){
                    if (item[i]>=x-40&&item[i]<=x+40){
                        Help_update_view.doHelp_update_view(2,i);
                    }
                }
                break;
        }
